window.onload = function () {
  let cars = _________._________("cars");
...
btnList.forEach((btn) => {
  btn.addEventListener("click", function (event) {
    ...
    let localCars = _________._________("cars");

    if (localCars === null) {
      localCars = [cars];
    } else {
      // cars가 있다면 Json 으로 변경
      localCars = JSON._________(localCars);
      localCars.push(cars);
    }

    // 로컬스토리지에 저장하기 위해서는 문자열의 형태로만 저장할 수 있다.
    const carsJson = JSON._________(localCars);
    _________._________("cars", carsJson);
  });
});
